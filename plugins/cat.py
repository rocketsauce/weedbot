...
def catte(inp, nick='', chan=None, say=None, db=None, reply=None):
    ...
    text = inp
    dir = "/var/www/slowmotioncats.com/cats/"
    r = random_file(dir)
    image = dir + r
    ...
    font = ImageFont.truetype("plugins/Impact.ttf", 55)
    bbox = (50, 0, imgWidth, imgHeight)

    drawtext(draw, text, font, "white", bbox)

    catName = randomFilename(suffix=".jpg")
    catDir = "/var/www/slowmotioncats.com/catption/"
    catImage = catDir + catName
    im.save(catImage)
    if inp == '':
        output = "http://slowmotioncats.com/cats/" + r
        say(output)
    else:
        output = "http://slowmotioncats.com/catption/" + catName + " - Caption added to BADCAT"
        say(output)
...
if __name__ == "__main__":
    ...
    def test():
        ...
        im.save(catImage)
        print catImage
        print "http://slowmotioncats.com/catption/" + catName
        #im.show()
