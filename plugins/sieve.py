...
def sieve_suite(bot, input, func, kind, args):
    ...
    if acl:
        ...
        if 'always-allow' in acl:
            explicit_allow = map(unicode.lower, acl['always-allow'])
            if input.chan.lower() in explicit_allow or "*" in explicit_allow:
                return input

    acl = bot.config.get('acls', {}).get("*")
    if acl:
        if 'deny-except' in acl:
            allowed_channels = map(unicode.lower, acl['deny-except'])
            if input.chan.lower()[0] == "#" and input.chan.lower() not in allowed_channels:
                return None
        if 'allow-except' in acl:
            denied_channels = map(unicode.lower, acl['allow-except'])
            if input.chan.lower() in denied_channels:
